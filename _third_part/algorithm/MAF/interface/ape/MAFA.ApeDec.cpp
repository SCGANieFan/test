...
#endif
	initParam.mode = ApeDecInitMode_e::APE_DEC_INIT_MODE_DEC;
...
maf_int32 MAFA_ApeDec::Process(MAF_Data* dataIn, MAF_Data* dataOut)
{
	...
	if (dataIn->CheckFlag(MAFA_FRAME_IS_EXTRA_INFO))
	{
		...
		initParam.mode = ApeDecInitMode_e::APE_DEC_INIT_MODE_DEC_USE_CONTEXT;
		initParam.contextInit_t.context = (void*)*(maf_uint32*)dataIn->GetData();
		initParam.contextInit_t.startFrame = *(maf_uint32*)(dataIn->GetData() + 4);
		maf_int32 ret = ApeDec_Init(_hd, &initParam);
		dataIn->Used(8);
		if (ret < 0){
			return -1;
