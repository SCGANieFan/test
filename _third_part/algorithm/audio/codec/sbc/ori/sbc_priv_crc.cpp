#include "sbc_priv.h"
extern "C" const unsigned char sbc_crc_tbl[256] = {
	
};

uint8_t sbc_4bit_crc_calc(uint8_t input,uint8_t fcs)
{
	int8_t i;
	uint8_t bit;
	uint8_t FCS = fcs;
	uint8_t shift;

	shift = 7;
	for (i = 0; i < 4; i++) {
		bit = (uint8_t)((0x01 & (input >> shift--)) ^ (FCS >> 7));
		if (bit) {
			FCS = (uint8_t)(((FCS << 1) | bit) ^ 0x1C);
		}
		else {
			FCS = (uint8_t)((FCS << 1));
		}
	}

	return FCS;
}

bool sbc_crc_check(sbc_info_t* info,void*buff){
	int n=3;///header
	if(SBC_CHNL_MODE_JOINT_STEREO == info->channelMode){
		n+=1;
	}
	//scale factor
	n+=info->numChannels*info->numSubBands/2;
	//
	uint8_t fcs = 0x0F;
	uint8_t*ptr = (uint8_t*)buff;
	fcs = sbc_crc_tbl[fcs ^ *ptr++];
	fcs = sbc_crc_tbl[fcs ^ *ptr++];
	//byte 3 is crc;
	uint8_t crc = *ptr++;
	uint8_t*ptrn = (uint8_t*)buff+n-1;
	while (ptr < ptrn) {
		fcs = sbc_crc_tbl[fcs ^ *ptr++];
	}
	if (SBC_CHNL_MODE_JOINT_STEREO == info->channelMode) {
		if (info->numSubBands == 8) {
			fcs = sbc_crc_tbl[fcs ^ *ptr++];
		}
		else if (info->numSubBands == 4) {
			uint8_t shift = 7;
			for (int i = 0; i < 4; i++) {
				unsigned char bit = (unsigned char)((0x01 & (*ptr >> shift--)) ^ (fcs >> 7));
				if (bit) {
					fcs = (uint8_t)(((fcs << 1) | bit) ^ 0x1C);
				}
				else {
					fcs = (uint8_t)((fcs << 1));
				}
			}
		}
	}
	else
	{
		fcs = sbc_crc_tbl[fcs ^ *ptr++];
	}
	return fcs == crc;
}


void sbc_crc_calc(sbc_info_t* info,void*buff){

	const unsigned char* tbls = sbc_crc_tbl;
	int bits=4*8;///header
	if(SBC_CHNL_MODE_JOINT_STEREO == info->channelMode){
		bits+=info->numSubBands;
	}
	//scale factor
	bits+=info->numChannels*info->numSubBands*4;
	//
	int n = (bits+7)>>3;
	//
	uint8_t fcs = 0x0F;
	uint8_t*ptr = (uint8_t*)buff+1;
	//ptr++;//skip syncwords
	fcs = tbls[fcs ^ *ptr++];//printf("%02x-%02x,",fcs,ptr[-1]);
	fcs = tbls[fcs ^ *ptr++];//printf("%02x-%02x,",fcs,ptr[-1]);
	//byte 3 is crc;
	uint8_t* crc = ptr++;
	//
	uint8_t* ptrn = (uint8_t*)buff + n - 1;
	while(ptr<ptrn){
		fcs = tbls[fcs ^ *ptr++];//printf("%02x-%02x,",fcs,ptr[-1]);
	}

	if (SBC_CHNL_MODE_JOINT_STEREO == info->channelMode) {
		if (info->numSubBands == 8) {
			fcs = sbc_crc_tbl[fcs ^ *ptr++];
		}
		else if (info->numSubBands == 4) {
			uint8_t shift = 7;
			for (int i = 0; i < 4; i++) {
				unsigned char bit = (unsigned char)((0x01 & (*ptr >> shift--)) ^ (fcs >> 7));
				if (bit) {
					fcs = (uint8_t)(((fcs << 1) | bit) ^ 0x1C);
				}
				else {
					fcs = (uint8_t)((fcs << 1));
				}
			}
		}
	}
	else
	{
		fcs = sbc_crc_tbl[fcs ^ *ptr++];
	}

	//dbgTestPDL(crc);
	//dbgTestPDL(fcs);
	//printf("crc:%02x/%02x,%d,%d,%p,%p,%p\n",fcs,Encoder->streamInfo.fcs,bits,n,buff,ptr,ptrn);
	*crc = fcs;
	return;
}