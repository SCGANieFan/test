...
static uint32_t range_dec_normalize(ApeRangecoder* rc, void* bufferRead)
{
	//check
	while (rc->range <= ((1u << (32 - 1)) >> 8))
	{
		rc->buffer <<= 8;
		rc->buffer += BufferRead(bufferRead, 1);
...
static bool range_get_symbol(ApeRangecoder* rc, const uint16_t counts[], const uint16_t counts_diff[], void* bufferRead, int32_t* symbol)
{
	int32_t cf;
	int32_t out;
...
static int32_t range_decode_bits(ApeRangecoder* rc, int n, void* bufferRead)
{
	int32_t sym;
	range_dec_normalize(rc, bufferRead);
	rc->range = rc->range >> n;
