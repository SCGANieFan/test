...
bool AudioSamples16::Init(uint8_t* buff, int16_t channels, int32_t samples, int16_t fixPointNum)
{
	...
	return AudioSamples::Init(buff, sizeof(int16_t), channels, samples, fixPointNum);
}

bool AudioSamples16::ProductWithEverySamlpe(AudioSamples& src, int32_t productSample)
{
#if 0
	if (productSample < 0)
		return false;
...
bool AudioSamples16::LeftShift(int16_t bits, bool isSigned)
{
	if (bits < 0
		|| bits > 32)
		return false;
...
bool AudioSamples16::RightShift(int16_t bits, bool isSigned)
{
	if (bits < 0
		|| bits > 32)
		return false;
...
bool AudioSamples16::AppendInFixPoint(int32_t num, int32_t den)
{
...
bool AudioSamples16::OverlapAdd(uint8_t* src, int32_t overlapSample)
{
	//check
	int32_t fixNum = 15;
	int32_t factor;
...
int16_t AudioSamples16::WaveFormMatch(uint8_t* comp, int32_t seekSample, int32_t matchSample)
{
